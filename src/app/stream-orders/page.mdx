...
<Note>
    Stream order classifications are established with reference to NIWA's River Environment Classification (REC); a database of catchment spatial attributes, summarised for every segment in New Zealand's network of rivers. See: https://niwa.co.nz/freshwater/management-tools/environmental-flow-tools/river-environment-classification
</Note>
